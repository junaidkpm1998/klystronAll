    ...
    def print_payroll_com_report(self):
        ...
        if not self.employee_id:
            ...
            for emp in employees:
                ...
                prev_number = []
                if prev_month_payslip:
                    for prev in prev_month_payslip:
                        prev_number.append(prev.number)
                prev_payslip_ref = ' , '.join([str(elem) for elem in prev_number])

                current_number = []
                if current_month_payslip:
                    for cur in current_month_payslip:
                        current_number.append(cur.number)
                current_payslip_ref = ' , '.join([str(elem) for elem in current_number])

                lines = {
                    'employee_name': emp.name,
                    'prev_payslip_ref': prev_payslip_ref,
                    'prev_basic_wage': sum(prev_month_payslip.mapped('basic_wage')),
                    'prev_net_wage': sum(prev_month_payslip.mapped('net_wage')),
                    'current_payslip_ref': current_payslip_ref,
        ...
        return self.env.ref('kg_hrms.action_payroll_comparison_report').report_action(self, data=data)
