...
class Wizard(models.TransientModel):
    ...
    def get_data(self, start, end):
        ...
        for slip in slips:
            if ids:
                ...
                ids = ids + str(slip.id)
        query = """select hr_employee.id, labour_card_number, salary_card_number, agent_id, hr_payslip_line.amount, hr_employee.name, hr_contract.wage from hr_employee
                    join hr_payslip_line on hr_employee.id = hr_payslip_line.employee_id
                    join hr_contract on hr_contract.employee_id = hr_employee.id
    ...
    def get_xlsx_report(self, lines, response):
        ...
        format1 = workbook.add_format({'font_size': 11, 'color': 'red', 'align': 'center','bg_color': 'yellow', 'bold': True})
        ...
        sheet.write(0,5, 'Payment Info', format1)
        sheet.set_column(2, 0, 15)
        sheet.set_column(2, 1, 15)
        sheet.set_column(2, 2, 15)
        sheet.set_column(2, 3, 15)
        sheet.set_column(2, 4, 15)
        sheet.set_column(2, 5, 30)
        sheet.set_column(2, 6, 15)
        sheet.set_column(2, 7, 15)
        sheet.set_column(2, 8, 15)
        sheet.set_column(2, 9, 15)
        sheet.set_column(2, 10, 15)
        sheet.set_column(2, 11,15)
        sheet.write(2,0, 'Emp ID', format1)
        sheet.write(2,1, 'Emp Resident ID',format1 )
        sheet.write(2,2, 'Emp Bank ID', format1)
        sheet.write(2,3, 'Emp Account No', format1)
        sheet.write(2,4, 'Emp Name', format1)
        sheet.write(2,5, 'Payment Amount', format1)
        sheet.write(2,6, 'Basic Salary',format1)
        sheet.write(2,7, 'Housing Allowance', format1)
        sheet.write(2,8, 'Other Earnings',format1)
        sheet.write(2,9, 'Deductions',format1)
        sheet.write(2,10, 'Regulatory Report',format1)
        sheet.write(2,11, 'Remarks',format1)
        ...
        for count in range(0, len(dat)):
            l +=1
            # here we fetch data on the base of 'House Allowance' and 'Profession Tax'
            payslip_line = self.env['hr.payslip.line'].search([('employee_id', '=', dat[count][0]), ('name', '=', 'House Rent Allowance')], limit=1)
            deduction = self.env['hr.payslip.line'].search([('employee_id', '=', dat[count][0]), ('name', '=', 'Professional Tax')], limit=1)
            payslip = self.env['hr.payslip.line'].search([('employee_id', '=', dat[count][0])], limit=1)
            bank = self.env['res.partner.bank'].search([('partner_id', '=', dat[count][0])], limit=1)
            ...
            sheet.write(l, 0, dat[count][0], format0)
            sheet.write(l, 1, 'Resident ID' , format0)
            sheet.write(l, 2, 'Bank ID', format0)
            sheet.write(l, 3, bank.acc_number, format0)
            sheet.write(l, 4, dat[count][5], format0)
            sheet.write(l, 5, dat[count][4], format0)
            sheet.write(l, 6, dat[count][6], format0)
            sheet.write(l, 7, payslip_line.amount, format0)
            sheet.write(l, 8, leaves, format0)
            sheet.write(l, 9, deduction.amount, format0)
            sheet.write(l, 10, leaves, format0)
            sheet.write(l, 11, payslip.note, format0)
        workbook.close()
        output.seek(0)
