...
class Probation(models.Model):
    ...
    def write(self, vals):
        ...
        for data in self:
            if data.state == 'probation':
                if vals.get('state') == 'open' and not data.is_approve:
                ...
                if vals.get('state') == 'cancel' or vals.get('state') == 'close' or vals.get('state') == 'draft':
                    raise UserError(_("You cannot change the status of non-approved Contracts"))
            training_dtl = self.env['hr.training'].search([('employee_id', '=', data.employee_id.id)])
            if training_dtl:
                return super(Probation, self).write(vals)
            if not training_dtl:
                if data.trial_date_end and data.state == 'probation':
                    self.env['hr.training'].create({
                        'employee_id': data.employee_id.id,
                        'start_date': data.date_start,
                        'end_date': data.trial_date_end,
                    })
            return super(Probation, self).write(vals)
