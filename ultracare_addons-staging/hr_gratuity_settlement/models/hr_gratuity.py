...
class EmployeeGratuity(models.Model):
        ...
        if self.employee_id.id:
            ...
            if not contract_sorted:
                raise UserError(_('No contracts found for the selected employee...!\n'
                                  'Employee must have at least one contract to compute gratuity settlement.'))
            ...
            print(hr_duration_config_ids, "HRRRRRR")
            print(self.total_working_years, "TOTTTTTT WORKINGGG")
            ...
            if gratuity_duration_id:
                self.employee_gratuity_duration = gratuity_duration_id
            else:
            ...
            if self.employee_gratuity_duration and self.wage_type == 'hourly':
                if self.employee_gratuity_duration.employee_working_days != 0:
                    ...
                    print('!!!!!!!', self.employee_gratuity_amount)
                else:
                ...
                if self.employee_gratuity_duration.employee_daily_wage_days != 0:
                    ...
                    print(daily_wage, "DAIly Wageee")
                    working_days_salary = daily_wage * self.employee_gratuity_duration.employee_working_days
                    print(working_days_salary, "working_days_salary")
                    gratuity_pay_per_year = working_days_salary * self.employee_gratuity_duration.percentage
                    print(gratuity_pay_per_year, "gratuity_per_year")
                    print(self.employee_gratuity_years, "!!!!!!!!!!!!!")
                    employee_gratuity_amount = gratuity_pay_per_year * self.employee_gratuity_years
                    print(employee_gratuity_amount, "GRatt")
                    self.employee_gratuity_amount = round(employee_gratuity_amount, 2)
                    print('@@@@@@@@', self.employee_gratuity_amount)
                else:
    ...
    def action_paid(self):
        for hr_gratuity_id in self:
            exp_account_id = int(
                self.env['ir.config_parameter'].sudo().get_param('hr_gratuity_settlement.gratuity_account_id'))
            emp_account = self.env['res.partner'].search([('name', '=', self.employee_id.name)])
            payable_account = emp_account.property_account_payable_id.id
