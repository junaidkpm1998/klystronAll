...
class BankPaymentRegisterReport(models.AbstractModel):
    _name = 'report.kg_ultracare_accounting.bank_payment_reg_template'
    _description = 'Bank Payment Register Report'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        account_payment_id = self.env['account.payment'].search(
            [('date', '>=', date_from),('date', '<=', date_end),('state', '!=', ['draft', 'cancel']),('journal_id.type','=','bank')])
        for vals in account_payment_id:
            date = vals.date
            doc_no = vals.name
            reference = vals.ref
            acc_date = vals.move_id.date
            account_code = ''
            account_name = ''
            description = ''
            credit = 0
