...
class ACAgingAnalysisReport(models.AbstractModel):
    _name = 'report.kg_ultracare_accounting.ac_aging_analysis_tem'
    _description = 'AC Aging Analysis Report'
    ...
    def _get_partner_move_lines(self, account_type, date_from, period_length, period_length_months,
        ...
        if period_length_type == 'days':
            if uneven:
                ...
                for i in range(range_count)[::-1]:
                    ...
                    period_name = str((4 - (i + 1)) * period_length + 1) + '-' + str((4 - i) * period_length)
                    period_stop = (start - relativedelta(days=1)).strftime('%Y-%m-%d')
                    if i == 0:
                        period_name = '+' + str(3 * period_length)
                    periods[str(i)] = {
                        'name': period_name,
                        'stop': period_stop,
                        'start': (i != 0 and stop.strftime('%Y-%m-%d') or False),
        ...
        for i in range(range_count):
            ...
            for line in self.env['account.move.line'].browse(aml_ids):
                ...
                # payment_lines = payment_obj.search(
                #     [('partner_id', '=', partner_id), ('date', '<=', date_from), ('payment_method_id.code', '=', 'pdc'),
                ...
                for rec in payment_lines:
                    line_pdc_amount = float_repr(abs(rec.price_subtotal),
                                                 rec.move_id.currency_id.decimal_places if rec.move_id.currency_id else 2)
                line_paid_amount = sum(payment.amount for payment in paid_payment_lines)
                partners_pdc_amount[partner_id] = float(line_pdc_amount)
                line_pdc_amount = 0
                partners_paid_amount[partner_id] = line_paid_amount

                if not self.env.user.company_id.currency_id.is_zero(line_amount):
                    partners_amount[partner_id] += line_amount
                    # partners_pdc_amount[partner_id] += line_pdc_amount
                    lines[partner_id].append({
                        'line': line,
                        'amount': line_amount,
                        'period': i + 1,
            ...
            history_pdc_amt.append(partners_pdc_amount)
            history_paid_amt.append(partners_paid_amount)
        ...
        for partner in partners:
            ...
            for i in range(range_count):
                ...
                if partner['partner_id'] in history[i]:
                    ...
                    pdc_ttl_amt = (during_pdc and during_pdc[0] or 0)
                    values['pdc_ttl_amt'] = pdc_ttl_amt

                if partner['partner_id'] in history_pdc_amt[i]:
                    during_pdc = [history_pdc_amt[i][partner['partner_id']]]
                    pdc_ttl_amt = (during_pdc and during_pdc[0] or 0)
                    values['pdc_ttl_amt'] = pdc_ttl_amt
                if partner['partner_id'] in history_paid_amt[i]:
                    payment = [history_paid_amt[i][partner['partner_id']]]
                    paid_ttl_amt = (payment and payment[0] or 0)
                    values['paid_ttl_amt'] = paid_ttl_amt
                if not 'paid_ttl_amt' in values.keys():
                    values['paid_ttl_amt'] = 0
                if not 'pdc_ttl_amt' in values.keys():
                    values['pdc_ttl_amt'] = 0
    ...
    def _get_report_values(self, docids, data=None):
        ...
        movelines, total, dummy = self._get_partner_move_lines(account_type, date_from,
