...
class CashBookReport(models.AbstractModel):
    _name = 'report.kg_ultracare_accounting.cash_boo_template_id'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for jou in journal_id:
            journal_list = {
                'type': 'journal',
                'journal_code': str(jou.default_account_id.code).upper(),
                'journal_name': str(jou.default_account_id.name).upper()
            }
            account_move_line_id = self.env['account.move.line'].search(
                [('date', '>=', date_from), ('date', '<=', date_end), ('move_id.state', '=', 'posted'),
                 ('journal_id.type', '=', 'cash'), ('journal_id', '=', jou.id)])
