...
class Wizard(models.TransientModel):
    ...
    def print_xlsx(self):
        ...
        if not company.bank_ids.bank_id.routing_code:
    ...
    def get_leaves(self, emp_id, start, end):
        leaves = self.env['hr.leave'].search([
            ...
            ('holiday_status_id', '=',
             4)])
        leave = sum(leaves.mapped('number_of_days'))
        return leave * -1
    ...
    def get_xlsx_report(self, lines, response):
        ...
        for count in enumerate(dat):
            ...
            sheet.write(count[0] + 1, 8, '', format0)
        ...
        sheet.write(count + 1, 0, 'SCR', format0)
        sheet.write(count + 1, 1, company.company_registry, format0)
        sheet.write(count + 1, 2, company.bank_ids.bank_id.routing_code, format0)
        sheet.write(count + 1, 3, lines['date'], format0)
        time = str(lines['date']).split(' ')[1].split(':')
        sheet.write(count + 1, 4, time[0] + ':' + time[1] + '- Time', format0)
        month_year = str(lines['end_date']).split('-')
        month_year = str(month_year[1]) + '/' + str(month_year[0]) + '- Month and Year'
        sheet.write(count + 1, 5, month_year, format0)
        sheet.write(count + 1, 6, '', format0)
        sheet.write(count + 1, 7, v_sum, format0)
        sheet.write(count + 1, 8, 'AED', format0)
        workbook.close()
        b = output.seek(0)
        a = response.stream.write(output.read())
        output.close()
